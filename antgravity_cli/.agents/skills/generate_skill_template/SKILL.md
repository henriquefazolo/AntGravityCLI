...
2. **Execute the Automation**:
   - First, find the absolute path of `generate_template.py`. You can use `FIND_FILE` or `SEARCH_DIR` to search for it (it resides inside the `antgravity_cli` folder under `.agents/skills/generate_skill_template/scripts/`).
